...
<body>
    ...
    <script type="text/babel">
        ...
        const App = () => {
            ...
            const [listaEspera, setListaEspera] = useState(JSON.parse(localStorage.getItem('maizena_lista')) || []);
            ...
            useEffect(() => {
                ...
                localStorage.setItem('maizena_lista', JSON.stringify(listaEspera));
            ...
            }, [produtos, vendas, listaEspera, config, funcionarios, userLogado]);
            ...
            const handleVenda = (id, cliente, pagamentosMultiplos) => {
                const p = produtos.find(item => item.id === id);
                if (p.estoque <= 0) return alert("Estoque esgotado!");

                const totalVenda = parseFloat(p.valor);
                ...
                const novaVenda = {
                    ...
                    pagamentos: pagamentosMultiplos, // Salva array de pagamentos
                    pagamento: pagamentosMultiplos.map(pg => pg.forma).join(" + "),
            ...
            const handleFinalizarCaixa = (itensCarrinho, cliente, telefone, pagamentosMultiplos, total, vendedor, paraLista) => {
                if (paraLista) {
                    const novosItensLista = itensCarrinho.map(item => ({
                    ...
                    setListaEspera([...listaEspera, ...novosItensLista]);
            ...
            const marcarRetiradaLista = (id) => {
                setListaEspera(listaEspera.map(item => 
                    item.id === id ? { ...item, retirado: true, dataSaida: new Date().toISOString() } : item
                ));
            };
                ...
                <div className="max-w-[1400px] mx-auto min-h-screen border-x border-zinc-800 flex flex-col">
                    <header className="p-6 flex flex-wrap gap-4 justify-between items-center border-b border-zinc-800 bg-[#111111] sticky top-0 z-40">
                        ...
                        <div className="flex bg-zinc-900 p-1 rounded-2xl border border-zinc-800">
                            ...
                            {temAcesso('caixa') && <button onClick={() => setAbaAtiva('lista')} className={`px-4 py-2 rounded-xl font-bold transition text-xs ${abaAtiva === 'lista' ? 'bg-maizena text-black' : 'text-zinc-500'}`}>Lista</button>}
                    ...
                    <main className="p-6 flex-1">
                        ...
                        {abaAtiva === 'caixa' && <ModuloCaixa produtos={produtos} funcionarios={funcionarios} onFinalizar={handleFinalizarCaixa} />}
                        {abaAtiva === 'lista' && <ModuloLista lista={listaEspera} setLista={setListaEspera} onRetirar={marcarRetiradaLista} />}
        ...
        const ModuloCaixa = ({ produtos, funcionarios, onFinalizar }) => {
            ...
            const addPagamento = (forma) => {
                ...
                if(resto <= 0) return;
                setPagamentos([...pagamentos, { forma, valor: resto.toFixed(2) }]);
            };

            const processar = (paraLista) => {
                if (!vendedorId) return alert("Selecione o vendedor.");
                if (paraLista && (!cliente || !telefone)) return alert("Nome e Telefone são obrigatórios para a lista.");
                
                const vend = vendedorId === 'admin' ? {id:'admin', nome:'ADM'} : funcionarios.find(f=>f.id===vendedorId);
                onFinalizar(carrinho, cliente, telefone, pagamentos, total, vend, paraLista);
                setCarrinho([]); setPagamentos([]); setCliente(''); setTelefone('');
            };
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
                    <div className="lg:col-span-2 space-y-6">
                        <div className="bg-zinc-900 p-6 rounded-[32px] border border-zinc-800">
                            <label className="block text-[10px] font-black text-maizena uppercase mb-2">Vendedor *</label>
                            <select value={vendedorId} onChange={e=>setVendedorId(e.target.value)} className="w-full bg-black border border-zinc-800 p-4 rounded-2xl text-white font-bold outline-none">
                                <option value="">SELECIONE...</option>
                                <option value="admin">ADMINISTRADOR</option>
                                {funcionarios.map(f => <option key={f.id} value={f.id}>{f.nome.toUpperCase()}</option>)}
                            </select>
                        ...
                        <div className="bg-zinc-900 p-6 rounded-[32px] border border-zinc-800">
                            <input value={busca} onChange={e=>setBusca(e.target.value)} placeholder="Pesquisar produto..." className="w-full bg-black border border-zinc-800 p-5 rounded-2xl text-white font-bold outline-none" />
                            {filtrados.length > 0 && (
                                <div className="bg-zinc-800 rounded-2xl mt-2 overflow-hidden">
                                    {filtrados.map(p => (
                                        <button key={p.id} onClick={() => addAoCarrinho(p)} className="w-full p-4 hover:bg-white/5 flex justify-between border-b border-zinc-700 last:border-0">
                                            <span className="font-bold text-sm uppercase">{p.nome}</span>
                                            <span className="text-maizena font-black">R$ {parseFloat(p.valor).toFixed(2)}</span>
                                        </button>
                                    ))}
                                </div>
                            )}
                        ...
                        <div className="bg-zinc-900 rounded-[32px] border border-zinc-800 overflow-hidden">
                            <div className="p-4 bg-black/40 font-black text-xs uppercase">Itens Selecionados</div>
                            {carrinho.map((item, idx) => (
                                <div key={idx} className="p-4 flex justify-between border-b border-zinc-800/50">
                                    <div className="flex gap-4">
                                        <button onClick={()=>setCarrinho(carrinho.filter((_,i)=>i!==idx))} className="text-soft-red"><Icon name="trash" size={16}/></button>
                                        <span className="text-xs font-bold uppercase">{item.nome}</span>
                                    </div>
                                    <span className="text-xs font-black">R$ {parseFloat(item.valor).toFixed(2)}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                    <div className="space-y-4">
                        <div className="bg-maizena p-6 rounded-[32px] text-black">
                            <p className="text-[10px] font-black uppercase opacity-60">Total Venda</p>
                            <h3 className="text-4xl font-black italic">R$ {total.toFixed(2)}</h3>
                        </div>
                        <div className="bg-zinc-900 p-6 rounded-[32px] border border-zinc-800 space-y-4">
                            <Input label="Nome do Cliente" value={cliente} onChange={setCliente} />
                            <Input label="Telefone / WhatsApp" value={telefone} onChange={setTelefone} />
                            
                            <div className="grid grid-cols-2 gap-2">
                                {FORMAS_PAGAMENTO.map(f => (
                                    <button key={f} onClick={() => addPagamento(f)} className="p-2 bg-black border border-zinc-800 rounded-xl text-[8px] font-black uppercase hover:border-maizena">{f}</button>
                                ))}
                            </div>

                            {pagamentos.map((p, idx) => (
                                <div key={idx} className="flex gap-2 items-center">
                                    <div className="flex-1 bg-black p-2 border border-zinc-800 rounded-xl flex justify-between items-center">
                                        <span className="text-[9px] font-black text-zinc-500 uppercase">{p.forma}</span>
                                        <div className="flex items-center gap-1">
                                            ...
                                            <input 
                                                ...
                                                className="bg-transparent text-maizena font-black text-xs w-16 text-right outline-none"
                                            />
                                        </div>
                                    </div>
                                    <button onClick={()=>setPagamentos(pagamentos.filter((_,i)=>i!==idx))} className="text-zinc-600"><Icon name="x" size={14}/></button>
                                </div>
                            ))}

                            <div className="grid grid-cols-1 gap-2 pt-4">
                                <button disabled={total <= 0 || totalPago < total || !vendedorId} onClick={() => processar(false)} className="w-full bg-tech-green text-black py-4 rounded-2xl font-black uppercase text-xs disabled:opacity-30">Vender e Retirar</button>
                                <button disabled={total <= 0 || !vendedorId} onClick={() => processar(true)} className="w-full bg-white text-black py-4 rounded-2xl font-black uppercase text-xs">Adicionar à Lista</button>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        const ModuloLista = ({ lista, setLista, onRetirar }) => {
            const filtrarRetirados = lista.filter(i => !i.retirado);
            const filtradosJaSaíram = lista.filter(i => i.retirado);

            return (
                <div className="max-w-6xl mx-auto space-y-8">
                    <div className="flex justify-between items-center no-print">
                        <h2 className="text-3xl font-black italic uppercase">Lista de Espera / Reserva</h2>
                        <button onClick={() => window.print()} className="bg-zinc-800 p-4 rounded-2xl border border-zinc-700 font-black text-xs uppercase flex items-center gap-2">
                            <Icon name="printer" size={16}/> Imprimir Lista Completa
                        </button>
                    </div>

                    <div id="print-lista" className="space-y-6">
                        <div className="bg-zinc-900 rounded-[32px] border border-zinc-800 overflow-hidden">
                            <div className="p-6 bg-maizena text-black font-black uppercase italic text-sm">Produtos em Espera</div>
                            <table className="w-full text-left">
                                <thead className="bg-black text-[10px] font-black uppercase text-zinc-500 border-b border-zinc-800">
                                    <tr>
                                        ...
                                        <th className="p-4 text-right no-print">Ação</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {filtrarRetirados.map(item => (
                                        <tr key={item.id} className="border-b border-zinc-800/50 text-xs">
                                            <td className="p-4 font-bold uppercase">{item.cliente} <br/> <span className="text-zinc-500 font-medium">{item.telefone}</span></td>
                                            <td className="p-4 font-black text-maizena uppercase">{item.produtoNome}</td>
                                            <td className="p-4 text-zinc-400 font-bold">{new Date(item.dataEntrada).toLocaleString('pt-BR')}</td>
                                            <td className="p-4"><span className="bg-soft-orange/20 text-soft-orange px-2 py-1 rounded-md font-black uppercase text-[9px]">Na Loja / Pendente</span></td>
                                            <td className="p-4 text-right no-print">
                                                <button onClick={() => onRetirar(item.id)} className="bg-tech-green text-black px-4 py-2 rounded-xl font-black uppercase text-[10px]">Retirar</button>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
                        ...
                        <div className="bg-zinc-900 rounded-[32px] border border-zinc-800 overflow-hidden opacity-60">
                            <div className="p-6 bg-zinc-800 text-white font-black uppercase italic text-sm">Histórico de Saídas</div>
                            <table className="w-full text-left">
                                <tbody>
                                    {filtradosJaSaíram.map(item => (
                                        <tr key={item.id} className="border-b border-zinc-800/50 text-xs">
                                            <td className="p-4 font-bold uppercase">{item.cliente}</td>
                                            <td className="p-4 uppercase">{item.produtoNome}</td>
                                            <td className="p-4 text-zinc-500">Entrou: {new Date(item.dataEntrada).toLocaleString('pt-BR')}</td>
                                            <td className="p-4 text-tech-green font-black">Saiu: {new Date(item.dataSaida).toLocaleString('pt-BR')}</td>
                                            <td className="p-4 text-right"><span className="text-[10px] font-black uppercase">Já Retirado</span></td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
        ...
        const ResumoDiario = ({ vendas, config, user }) => {
            ...
            const dados = useMemo(() => {
                ...
                // Cálculo por forma de pagamento
                const resumoFinanceiro = {};
                FORMAS_PAGAMENTO.forEach(forma => resumoFinanceiro[forma] = 0);

                filtradas.forEach(v => {
                    if (v.pagamentos) {
                        v.pagamentos.forEach(pg => {
                            if (resumoFinanceiro[pg.forma] !== undefined) {
                                resumoFinanceiro[pg.forma] += parseFloat(pg.valor || 0);
                            }
                        });
                    } else if (v.pagamento) {
                        // Fallback para vendas antigas que não tinham array
                        if (resumoFinanceiro[v.pagamento] !== undefined) {
                            resumoFinanceiro[v.pagamento] += parseFloat(v.valor || 0);
                        }
                    }
                });
                ...
                return { 
                    lista: filtradas, 
                    qtd: filtradas.reduce((a, v) => a + (v.isMulti ? v.itens.length : 1), 0), 
                    total: filtradas.reduce((a, v) => a + v.valor, 0),
            ...
            return (
                <div className="max-w-5xl mx-auto space-y-6">
                    <div className="flex justify-between items-end mb-4">
                        <div>
                            <h2 className="text-4xl font-black italic uppercase tracking-tighter">Resumo de Caixa</h2>
                    ...
                    {/* Novo Bloco: Detalhamento por Forma de Pagamento */}
                    <div className="bg-zinc-900 border border-zinc-800 rounded-[40px] p-8">
                        <h3 className="text-xl font-black italic uppercase mb-6 text-zinc-400">Entradas por Tipo</h3>
                        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                            {Object.entries(dados.financeiro).map(([forma, valor]) => (
                                <div key={forma} className="bg-black/50 p-4 rounded-3xl border border-zinc-800">
                                    <p className="text-[9px] font-black text-zinc-500 uppercase mb-1">{forma}</p>
                                    <p className={`text-sm font-black ${valor > 0 ? 'text-maizena' : 'text-zinc-700'}`}>R$ {valor.toFixed(2)}</p>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="bg-zinc-900 border border-zinc-800 rounded-[40px] overflow-hidden">
                        <table className="w-full text-left">
                            <thead className="bg-black/50 text-[10px] font-black uppercase text-zinc-500">
                                <tr><th className="p-8">Roupa</th><th className="p-8">Cliente</th><th className="p-8">Pagamento</th><th className="p-8 text-right">Valor</th></tr>
                            </thead>
                    ...
                    <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-[40px] w-full max-w-2xl relative shadow-2xl">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div className="border-2 border-dashed border-zinc-700 rounded-3xl flex flex-col items-center justify-center bg-black/40 overflow-hidden relative min-h-[250px]">
                                {f.imagem ? <img src={f.imagem} className="absolute inset-0 w-full h-full object-cover" /> : <Icon name="camera" className="text-zinc-600" size={40} />}
                                <input type="file" onChange={handleImg} className="absolute inset-0 opacity-0 cursor-pointer" />
                            </div>
                        </div>
                        <div className="flex gap-4 mt-8">
                            <button onClick={() => onSave(f)} className="flex-1 bg-tech-green text-black font-black py-5 rounded-2xl uppercase">SALVAR</button>
                            <button onClick={onClose} className="px-8 text-zinc-500 font-bold uppercase">CANCELAR</button>
                        </div>
                    </div>
                </div>
            );
        ...
        const ModalVenda = ({ produto, onConfirm, onClose }) => {
            ...
            const totalVenda = parseFloat(produto.valor);
            const totalPago = pagamentos.reduce((acc, p) => acc + parseFloat(p.valor || 0), 0);
            const resto = totalVenda - totalPago;
            ...
            return (
                <div className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-4">
                    <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-[40px] w-full max-w-md relative shadow-2xl">
                        <button onClick={onClose} className="absolute top-6 right-6 text-zinc-500 hover:text-white transition"><Icon name="x" size={24}/></button>
                        <h2 className="text-2xl font-black mb-6 uppercase text-maizena text-center italic">Retirada Direta</h2>
                        
                        <div className="bg-black/30 p-4 rounded-2xl border border-zinc-800 mb-6 text-center">
                            <p className="font-bold text-white uppercase">{produto.nome}</p>
                            <p className="font-black text-maizena">R$ {totalVenda.toFixed(2)}</p>
                            {resto > 0 && <p className="text-[10px] text-soft-red font-black mt-1">FALTANDO: R$ {resto.toFixed(2)}</p>}
                        </div>

                        <div className="space-y-4">
                            <Input label="Nome do Cliente" value={c} onChange={setC} />
                            
                            <p className="text-[10px] font-black text-zinc-500 uppercase ml-1">Forma de Pagamento (Pode ser mais de uma)</p>
                            <div className="grid grid-cols-2 gap-2">
                                {FORMAS_PAGAMENTO.map(forma => (
                                    <button 
                                        ...
                                        className="p-3 rounded-xl text-[9px] font-black border transition bg-black text-zinc-500 border-zinc-800 hover:border-maizena disabled:opacity-20"
                                    >
                                        {forma}
                                    </button>
                                ))}
                            ...
                            <div className="space-y-2">
                                {pagamentos.map((pg, idx) => (
                                    <div key={idx} className="flex gap-2 items-center">
                                        <div className="flex-1 bg-black p-3 border border-zinc-800 rounded-xl flex justify-between items-center">
                                            <span className="text-[9px] font-black text-zinc-400 uppercase">{pg.forma}</span>
                                            <input 
                                                type="number" 
                                                value={pg.valor} 
                                                onChange={(e) => {
                                                ...
                                                className="bg-transparent text-maizena font-black text-xs w-20 text-right outline-none border-b border-zinc-800"
                                            />
                                        </div>
                                        <button onClick={() => setPagamentos(pagamentos.filter((_,i)=>i!==idx))} className="text-soft-red p-1"><Icon name="trash" size={14}/></button>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <button 
                            ...
                            className={`w-full py-5 rounded-2xl font-black mt-8 transition-all ${totalPago >= totalVenda ? 'bg-tech-green text-black scale-105' : 'bg-zinc-800 text-zinc-600 opacity-50'}`}
                        >
                            CONFIRMAR RETIRADA
                        </button>
                    </div>
            ...
            <div className="fixed inset-0 bg-black/98 z-50 flex items-center justify-center p-4">
                <div className="bg-white p-8 w-[350px] text-black shadow-2xl relative" id="receipt-print">
                    <div className="text-center border-b-2 border-dashed border-gray-300 pb-6 mb-4">
                        ...
                        <p className="text-[10px] font-bold text-gray-400">Cupom de Retirada</p>
                    </div>
                    ...
                    <div className="mb-4">
                        <p className="text-[8px] font-black uppercase text-gray-400 mb-1">Pagamento Detalhado:</p>
                        {venda.pagamentos && venda.pagamentos.map((p, idx) => (
            ...
            return (
                <div className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-4">
                    <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-[40px] w-full max-w-md shadow-2xl relative">
                        <button onClick={onClose} className="absolute top-6 right-6 text-zinc-500 hover:text-white transition"><Icon name="x" size={24}/></button>
                        <h2 className="text-2xl font-black mb-8 text-maizena italic uppercase">Configuração</h2>
        ...
        const ModalIncluir = ({ produto, onConfirm, onClose }) => {
            ...
            return (
                <div className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-4">
                    <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-[40px] w-full max-sm text-center relative">
                        <button onClick={onClose} className="absolute top-6 right-6 text-zinc-500 hover:text-white transition"><Icon name="x" size={24}/></button>
                        <h2 className="text-xl font-black text-zinc-400 mb-6 uppercase">REPOR ESTOQUE</h2>
                        ...
                        <button onClick={() => onConfirm(produto.id, q)} className="w-full bg-tech-green text-black font-black py-5 rounded-2xl uppercase">ADICIONAR</button>
                    </div>
                </div>
            );
        };
                    ...
                    <div className="flex-1 overflow-y-auto space-y-3">
                        {vendas.map(v => (
                            <div key={v.id} className="bg-black border border-zinc-800 p-4 rounded-2xl flex justify-between items-center">
                                ...
                                <button onClick={() => onConfirm(v.id)} className="bg-soft-orange text-black px-4 py-2 rounded-xl font-black text-xs uppercase">Estornar</button>
                            </div>
                        ))}
                    </div>
                </div>
